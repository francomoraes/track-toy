...
describe('RotaryPlatform', () => {
  ...
  it('isAligned retorna true quando angulo corresponde a saida valida', () => {
    ...
    const aligned = createRotaryPlatform({ id: 'r1', exitAngle: 90, initialAngle: 90 });
...
describe('calculateScore', () => {
  ...
  it('contabiliza apenas eventos de tipo car_blocked como erros', () => {
    const events = [
      ...
      { type: 'car_blocked', timestamp: 500, data: { reason: 'wrong_sequence' } },
...
describe('useGameStore', () => {
  ...
  it('triggerCommand registra evento command_triggered', () => {
    ...
    expect(events).toContainEqual(
      expect.objectContaining({ type: 'command_triggered', data: { commandId: 'cmd-1' } })
    );
  });
...
describe('useGameStorage (modo local, sem auth)', () => {
  ...
  it('saveSession atualiza bestStars somente se resultado for melhor', () => {
    const { result } = renderHook(() => useGameStorage());
    act(() => result.current.saveSession({ levelId: 'level-1-1', stars: 2, completed: true, durationMs: 30_000, errors: 1, events: [] }));
    act(() => result.current.saveSession({ levelId: 'level-1-1', stars: 1, completed: true, durationMs: 60_000, errors: 3, events: [] }));
...
test('fase 1-1: elevar plataforma e completar ciclo', async ({ page }) => {
  ...
  await expect(page.getByTestId('level-complete')).toBeVisible({ timeout: 10_000 });
...
test('fase 1-3: elevar plataforma e depois abrir ponte em sequencia', async ({ page }) => {
  ...
  await expect(page.getByTestId('level-complete')).toBeVisible({ timeout: 15_000 });
});
```
